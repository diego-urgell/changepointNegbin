...
void binseg(cost_func, sumstat, n, m, pen, Q, cptsout, error, minorder, optimalorder, maxorder, minseglen, likeout,
            op_cps, shape, tol, MBIC)
        ...
        int *MBIC;          // 1 if MBIC penalty, 0 if not
{
    ...
    if (strcmp(*cost_func, "var.norm") == 0) {
        ...
        costfunction = &meanvar_poisson;
    } else if (strcmp(*cost_func, "negbin") == 0) {
        costfunction = &negbin_loss;
    }
    ...
    for (q = 0; q < *Q; q++) {
        ...
        costfunction(sumstat, &size, &np1, &l, minorder, optimalorder, maxorder, &start, &end, &cost, tol, error, shape,
                     MBIC);
        null = (-0.5) * cost;
        ...
        for (j = 2; j < (*n - 2); j++) {
            if (j == end) {
                ...
                costfunction(sumstat, &size, &np1, &l, minorder, optimalorder, maxorder, &start, &end, &cost, tol,
                             error, shape, MBIC);
                null = (-0.5) * cost;
            } else {
                if (((j - start - 1) >= *minseglen) && ((end - j) >= *minseglen)) {
                    double cost1 = 0;
                    double cost2 = 0;
                    costfunction(sumstat, &size, &np1, &p, minorder, optimalorder, maxorder, &start, &j, &cost1, tol,
                                 error, shape, MBIC);
                    costfunction(sumstat, &size, &np1, &p, minorder, optimalorder, maxorder, &j, &end, &cost2, tol,
                                 error, shape, MBIC);
